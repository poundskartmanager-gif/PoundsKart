exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    return { statusCode: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }, body: JSON.stringify({ collections: [], vendors: ['PoundsKart'], tags: [] }) };
  }
  ...
  try {
    ...
    if (!SHOPIFY_STORE || !SHOPIFY_TOKEN) {
      return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ error: 'Shopify credentials not set.' }) };
    }
    ...
    if (!resp.ok) return { statusCode: resp.status, headers: corsHeaders, body: JSON.stringify({ error: JSON.stringify(result.errors) }) };
